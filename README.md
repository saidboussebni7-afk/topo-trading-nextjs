...
- `/sojod` و `/admin` تم تعطيلهما عمداً.
...
## إصلاح 404 للوحة الأدمن

رابط لوحة الأدمن الصحيح في هذه النسخة هو:

```txt
/vip-admin-6d8f2a
```

تم وضع ملف لوحة الأدمن داخل:

```txt
public/vip-admin-6d8f2a/index.html
```

وتم تعطيل المسارات القديمة:
...
يجب نشر المشروع كاملاً على استضافة تدعم Next.js مثل Vercel، لأن الحماية تعتمد على `pages/api/*`. رفع مجلد `public` فقط سيجعل الواجهة تظهر أحياناً، لكن تسجيل دخول الأدمن وعمليات الأكواد لن تعمل.
